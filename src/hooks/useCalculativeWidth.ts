 ...
 */
export const useCalculativeWidth = (
	...
	useEffect(() => {
		...
		const throttledCountWidth = _.throttle(countWidth, 3000); // 节流

		window.addEventListener('resize', throttledCountWidth);
		return () => window.removeEventListener('resize', throttledCountWidth);
	}, []);
	return itemWidth;
};
